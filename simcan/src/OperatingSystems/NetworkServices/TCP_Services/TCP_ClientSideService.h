...
class TCP_ClientSideService : public TCPSocket::CallbackInterface{
	...
	*/
	struct SIMCAN_TCP_Client_Connector{
		TCPSocket *socket;
		cPacket *msg;
	};
	typedef struct SIMCAN_TCP_Client_Connector clientTCP_Connector;	
	...
	public:
	     ...
	     */
	    void createConnection(SIMCAN_Message *sm);	    
	    
	    /**
	     * Send a packet to corresponding destination.
	     * @param sm Message to send to the corresponding server.
	     */
	    void sendPacketToServer(SIMCAN_Message *sm);        
	       
	    /**
	     * Close a connection.	     
	     */
	    void closeConnection(SIMCAN_Message *sm);
