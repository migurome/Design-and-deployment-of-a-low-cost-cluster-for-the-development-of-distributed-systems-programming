...
void TCP_ClientSideService::createConnection(SIMCAN_Message *sm){
	...
	SIMCAN_App_NET_Message *sm_net;
		...
		// Wrong message?
		if (sm_net == NULL)
			networkService->showErrorMessage ("[createConnection] Error while casting to SIMCAN_App_NET_Message!");	
...
void TCP_ClientSideService::sendPacketToServer(SIMCAN_Message *sm){
...
void TCP_ClientSideService::closeConnection(SIMCAN_Message *sm){
...
//
//	 		// Cast
//			sm_net = dynamic_cast<SIMCAN_App_NET_Message *>(connections[index].msg);
//
//			// Try again!ç
