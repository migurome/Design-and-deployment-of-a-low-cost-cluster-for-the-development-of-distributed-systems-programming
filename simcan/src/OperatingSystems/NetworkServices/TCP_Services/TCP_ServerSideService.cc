...
TCP_ServerSideService::TCP_ServerSideService(string newLocalIP,			    										    						
				    						cGate* toTCP,
				    						cGate* toServiceRedirector,
				    						NetworkService *netService){	

		// Init...
		localIP = newLocalIP;	    	    
	    outGate_TCP = toTCP;
	    outGate_ServiceRedirector = toServiceRedirector;
	    networkService = netService;	    	    
}
...
void TCP_ServerSideService::newListenConnection (SIMCAN_Message *sm){
	...
	serverTCP_Connector newConnection;
		...
		// Wrong message?
		if (sm_net == NULL)
			networkService->showErrorMessage ("[TCP_ServerSideService::newListenConnection] Error while casting to SIMCAN_App_NET_Message!");
...
void TCP_ServerSideService::sendPacketToClient(SIMCAN_Message *sm){
...
void TCP_ServerSideService::socketDataArrived(int connId, void *yourPtr, cPacket *msg, bool urgent){
	...
	int appIndex;		
		...
		// Casting
		sm = dynamic_cast<SIMCAN_Message *>(msg);
		
		if (sm == NULL)
			networkService->showErrorMessage ("[socketDataArrived] Error while casting to SIMCAN_Message!");
	...
	networkService->sendRequestMessage (sm, outGate_ServiceRedirector);
}
