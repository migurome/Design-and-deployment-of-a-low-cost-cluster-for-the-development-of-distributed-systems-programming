...
void SMS_RAID_0::splitRequest (cMessage *msg){
	...
	SIMCAN_BlockList_Message *subRequestMsg;		// SubRequests message!

	vector <SIMCAN_BlockList_Message *> msgVector;	// Vector that contains the stripped requests!
...
string SMS_RAID_0::requestToStringByIndex (unsigned int index){
	...
	int i;
	int numSubRequest;
	SIMCAN_BlockList_Message *sm_subReq;
	SIMCAN_BlockList_Message *sm_bl;
		...
		if ((index>=requestVector.size()) || (index<0)){
			...
			// Cast!
			sm_bl = check_and_cast<SIMCAN_BlockList_Message *>((requestVector[index])->getParentRequest());
			...
			for (i=0; i<numSubRequest; i++){
				...
				if ((requestVector[index])->getSubRequest(i) == NULL)
					...
					// Cast!
					sm_subReq = check_and_cast<SIMCAN_BlockList_Message *>((requestVector[index])->getSubRequest(i));
