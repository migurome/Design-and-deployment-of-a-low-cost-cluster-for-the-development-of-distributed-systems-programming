...
int SplittingMessageSystem::searchRequest (cMessage* request){
	...
	bool found;
	int index;
	vector <SIMCAN_Request*>::iterator reqIterator;
...
void SplittingMessageSystem::addRequest (cMessage* request, unsigned int numSubRequests){
	...
	SIMCAN_Request *newRequest;
	
		newRequest = new SIMCAN_Request (request, numSubRequests);
...
SIMCAN_Message* SplittingMessageSystem::popSubRequest (cMessage* parentRequest, int subRequestIndex){
...
SIMCAN_Message* SplittingMessageSystem::popNextSubRequest (cMessage* parentRequest){
...
void SplittingMessageSystem::arrivesSubRequest (cMessage* subRequest, cMessage* parentRequest){
	...
	int indexSubRequest;
		...
		// Cast to SIMCAN_Message
		subReqMsg = check_and_cast<SIMCAN_Message *>(subRequest);
