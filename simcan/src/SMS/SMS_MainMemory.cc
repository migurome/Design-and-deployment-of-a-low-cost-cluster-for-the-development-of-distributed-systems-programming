...
SMS_MainMemory::SMS_MainMemory(unsigned long long int newMemorySize,
...
void SMS_MainMemory::splitRequest(cMessage *msg){
	...
	SIMCAN_App_IO_Message *subRequestMsg;		// SubRequest message
	SIMCAN_App_MEM_Message *sm_mem;				// SubRequest message
	SIMCAN_App_MEM_Message *subRequestMemMsg;	// SubRequest message
		...
		sm_io = dynamic_cast<SIMCAN_App_IO_Message *>(msg);
		...
		// Mem message?
			...
			sm_mem = dynamic_cast<SIMCAN_App_MEM_Message *>(msg);

			if (sm_mem != NULL){

				addRequest (msg, 1);

				subRequestMsg = (SIMCAN_App_IO_Message *) sm_mem->dupGeneric();
				subRequestMsg->addRequestToTrace (currentSubRequest);
				subRequestMsg->setParentRequest (msg);
				subRequests.push_back (subRequestMsg);
			}
...
bool SMS_MainMemory::arrivesRequiredBlocks(cMessage* request, unsigned int extraBlocks){
	...
	bool allArrived;
		...
		if (parentIndex != NOT_FOUND){
			...
			while ((subIndex < ((requestVector[parentIndex])->getNumSubRequest()-extraBlocks)) && (allArrived)){
...
SIMCAN_App_IO_Message* SMS_MainMemory::getFirstSubRequest(){
...
SIMCAN_App_IO_Message* SMS_MainMemory::popSubRequest(){
...
string SMS_MainMemory::requestToStringByIndex (unsigned int index){
	...
	SIMCAN_App_IO_Message *sm_io;
		...
		// Request found!
			...
			sm_io = dynamic_cast<SIMCAN_App_IO_Message *>((requestVector[index])->getParentRequest());
...
void SMS_MainMemory::clear (){	
	...
	std::list<SIMCAN_App_IO_Message*>::iterator iter;	
	int i;	
